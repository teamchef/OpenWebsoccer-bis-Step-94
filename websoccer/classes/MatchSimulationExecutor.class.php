...
class MatchSimulationExecutor {
	...
	public static function simulateOpenMatches(WebSoccer $websoccer, DbConnection $db) {
		...
		// Start - geblockte Spiele automatisch wieder fortsetzen
		// Ist der Haken im Admincenter gesetzt ( Standard ), so muss man die Spiele nicht von Hand entblocken.
		
		$autogames = $websoccer->getConfig('webjobexecution_enabled');
		if (!$autogames == 1) {
			$whereCondition = 'M.berechnet != \'1\' AND M.blocked != \'1\' AND M.datum <= %d ORDER BY M.datum ASC';
		}
		
		// Ende - geblockte Spiele automatisch wieder fortsetzen by ErdemCan
